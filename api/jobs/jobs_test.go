...
func TestCreateJobHandler(t *testing.T) {
	t.Run("successfully creates a job", func(t *testing.T) {
		...
		companyStore := &tests.StubCompanyStore{
			Company: companies.Company{ID: uuid.New(), Name: "Acme Corp", UserID: UserID},
		}
	...
	t.Run("company not found returns 404", func(t *testing.T) {
		...
		companyStore := &tests.StubCompanyStore{NotFound: true}
	...
	t.Run("store returns generic error (500)", func(t *testing.T) {
		jobStore := &tests.StubJobStore{Fail: true}
		categoryStore := &tests.StubCategoryStore{
			Categories: []categories.Category{{ID: uuid.New(), Name: "Engineering"}},
		}
		companyStore := &tests.StubCompanyStore{
			Company: companies.Company{ID: uuid.New(), Name: "Acme Corp"},
		}
		...
		req := createJobRequest(data, companyStore.Company.ID)
		rec := httptest.NewRecorder()
